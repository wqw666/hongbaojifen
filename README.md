...
| 游戏记录 | 游戏对局列表（群/玩法/局号/时间/会员数/净积分/异常数）+ 页头**总抽水/对局数（跟随当前筛选）**；详情含逐条**回放时间线**（每事件带流水额）；默认保留 30 天（配置键 `game_record_retention_days` 可改） |
...
| `GET /api/open/groups?keyword=&status=` | QQ 群列表（status=`active\|banned` 过滤）。每行含经营数据：`points_total`（群内会员当前积分合计）、`game_count`/`rake_total`（累计对局数/累计抽水，结算入账时累加）、`game_count_30d`/`rake_total_30d`（近 30 天实时值；累计值不随记录清理减少） |
...
管理端（JWT）配套接口：`GET/POST/PUT/DELETE /api/admin/executors`、`POST /api/admin/executors/{id}/reset-token|ban|unban`（ban 请求体可带 `reason`；解封不换 token）、`GET /api/admin/qq-accounts`、`POST/PUT/DELETE /api/admin/qq-accounts`（PUT 可改 `can_manual_points: allowed|denied`）、`POST/GET /api/admin/executors/{id}/commands`、`GET /api/admin/game-records?group_id=&play_name=&page=&size=`（`data.rake_total` = 当前筛选下总抽水）、`GET /api/admin/game-records/{id}`（含该局完整回放 events，事件带 `flow_amount` 流水额）、`GET /api/admin/members?keyword=&status=&group_id=`（按来源群过滤，群管理点群名开成员抽屉用）、`GET /api/admin/qq-groups`（同 open 群列表，含累计/近30天/积分合计）。
...
events 每条（≤2000/局）：{qq, nickname?, msg?, reply?, delta, flow?}   ← 消息级明细
...
- **积分与流水分离**（V1.0.13）：`delta` 决定积分变动，`flow` 只记流水额（不传 = `delta`，即全额）。
  撑庄模式按「下注半额」记流水（挑战者 `下注//2`、庄家 = 各挑战者之和，恒正、两侧对等；下注 100 → 玩家侧 50 + 庄家侧 50），
  平局 `delta=0` 但有流水 → 落一条 `type=FLOW` 的流水（不动积分）；大吃小不传 `flow`（按全额）。
