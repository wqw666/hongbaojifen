...
- **`D:\work\hongbaojifen\部署包-2026-09-16.zip`（147MB，另有同名可拷贝目录）——勿提交 git**。内容：`后端/`（jar 密码 123456、jlink JRE 78MB 含 jdk.charsets、start.bat/start.sh、启动后端.bat、建库.sql、VERSION）+ `agent执行器/`（agent.exe **2026.09.16-02** md5 `12875927…`：强制绕过系统代理 + 玩法页群选择修复 + A−/A+ 字体缩放；全量 release 含 tools/NapCat）+ 部署说明.txt。**同名可拷贝目录若被运行中的 agent.exe 锁住，删不掉**——先关 agent 再刷新目录（zip 不受影响）。**连 `dist\agent.exe` 被运行中的实例锁住时 `build.bat` 也会静默「打包失败」且退出码仍 0**（PyInstaller `os.remove` 拒绝访问）——构建前先 `tasklist` 清 agent.exe，日志 grep「打包失败」。
...
- **2026-09-18 打包在桌面 `C:\Users\wqwpc\Desktop\部署包-2026-09-18.zip`**（密码 123456，jar `c9210348` + agent `12875927`，其余同 -16）。注意：`部署说明.txt` 内要点列表是**3 空格缩进**（原始版）——2 空格的老字符串替换会静默失配，改文案前先 `repr()` 原文。
...
23. **同一消息的转发与回复走两条通道，通道的过滤条件必须一致**（2026-09-21）：上分申请「群里回了『申请已提交』但审批页没单」——插件的 `handlePlayMessage`（玩法转发→agent 回复确认语）放行「玩法群或监控群任一」，而 `handleApproveMessage`（审批转发→审批页）旧口径是「玩法群优先，否则监控群」→ **玩法群非空时监控群的申请被静默丢弃**，回复照发。教训：一个用户可见的确认（回复）和它的数据落地（审批单）如果走两条 fetch，两条路径的群过滤/超时/重试策略必须逐行对齐，改一条必须 grep 另一条。修复还含：审批 fetch 失败重试一次；`_on_callback_approve` 不再静默吞异常（全部 `_flog` 留痕）；`approve_tab` 审批行落盘 `approve_rows.json`（重启不丢）。
24. **worker 线程里 `self.after()` 是静默丢单重灾区**（2026-09-22 实测逮住）：tkinter 的 `after()` 从非主线程调用会抛 `RuntimeError('main thread is not in main loop')`（主线程不在 mainloop 的窗口期：启动中、messagebox 弹窗、登录/视图切换），而代码里到处是 `try: self.after(0, …) except Exception: pass` 的吞异常模式 → 排队的申请/表格刷新静默丢失且无任何痕迹。`approve_tab` 已改为**纯 queue + 主线程 after 自续轮询**（worker 只做纯数据 `_build_row`，`_ui_poll` 在主线程 drain）；play_tab/backend_tab 的同类模式目前只影响日志/表格刷新（非资金路径），暂未改，但**新代码禁止再在 worker 线程直接 after()**。判定法：`threading.Thread(target=...)` 里若出现 `self.after`，先查主线程窗口期。功能测试（%TEMP%/test_fix*.js|py）已把这几个修复做成可重跑用例：插件过滤矩阵 8 项、审批持久化链路 7 项、玩法激活 9 项。
25. **PyInstaller onefile 关闭告警「Failed to remove temporary directory」= 子进程从 _MEI 加载了 DLL**（2026-09-22 实测定位）：bootloader 把解包目录 `_MEIxxxx` 加进 PATH；agent 拉起的子进程链（wscript→cmd→NapCat→QQ）继承后，`VCRUNTIME140.dll` 命中 _MEI 副本并**常驻映射**（用 `Get-Process | Where { $_.Modules.FileName -like '*_MEI*' }` 可抓到持有者）→ 退出时 rmtree 失败弹窗 + TEMP 堆积 `_MEI*` 残留。修复：`gui/main.py::main`（frozen 时）剥掉 PATH 里含 `_MEI` 的段 + `os.chdir(exe目录)`。**坑中坑**：`main()` 内不得再有局部 `import os`（晚于引用 → UnboundLocalError，首版修复就这么挂的）。已堆积的旧 `_MEI*` 要等 NapCat/QQ 重启后才能删。
26. **CTk 6.0 的 `set_widget_scaling` 不改已有字体**（2026-09-22 实测 15→15）：只改控件尺寸/窗口。字体缩放按钮必须手动遍历控件树等比 `configure(size=...)`（tk 字号只收整数；按 `id(font)→原始字号` 基准 map 防漂移），另调 ttk Treeview 全局样式（ttk 不受 CTk 缩放影响）。
