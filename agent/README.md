...
| 路径 | 说明 |
|------|------|
| `gui/` | Python + CustomTkinter 桌面端（扫码登录；主界面 7 个 tab：群管理/会员/游戏玩法/积分审批/实时监控/总后台对接/更多）。群管理表格含**积分剩余/累计抽水/总对局**三列（`累计（近30天 X）` 格式，点「刷新」更新；未上报到总后台的群该列为空） |
...
红包事件   → RedPacketGame.handle_claim（只认本局第 1 个管理员红包）→ 逐领取人静默记账（v3.1 起正常领取
          不 @ 不回执，金额异常才回警告文本提示修正；领取全程无群播报）
          → 红包总份数 < 需开奖人数 → 自动作废播报（积分未扣，普通消息不带 @全体）
结算（面板按钮，worker 线程）→ 玩法 settle_redpacket 逐人开奖算 delta（未领者按 0 点）
          → POST /api/open/games/report {round_id, ...} → 总后台逐事件入账（round_id 幂等）
          → 播报「本局结果，总分N分」+ 结算表图（普通消息不带 @全体；v3.1 起无逐人 @ 开奖）
```
...
3. 「当前群」选群 → 「开始本局」→ 群内只播报一条 `————开始————`（普通消息不带 @全体，无局号/其他介绍；局号后台纯数字递增 0000001 起，R7 起不带玩法名前缀，`game_round_seq` 持久递增）
4. 群内玩法：成员直接发**数字下注**（如 500；旧词「下注500」兼容）；发**「撑」即可撑庄**（先到先得，已下注的不能再撑）；发「上100/下100」提交上/下分申请（进「积分审批」页）；群内发「开始游戏/开局」只引导去面板（Ruling：不开局）
5. 「停止下注」封盘：播报（普通消息不带 @全体）`————停结———— / 本局玩法：撑庄（庄家：X）或大吃小 / 合计 N 分` + **下注汇总表格图**（R7 起用图片整齐展示；尾部「请管理员发红包开奖（每份…）」引导已移除，开红包指引只在玩法日志提示管理员）；**无进行中的局**点停注 = 只记玩法日志、不发群；**撑庄局先查庄家余额**（需 ≥ 下注池+抽水，不足自动作废播报，积分未扣）
6. 管理员在群内发**红包**开奖 → 成员抢到静默记账（不逐人回执打扰；金额异常才回警告文本）；红包份数不足 → 自动作废播报；**第 2 个管理员红包被忽略**（只认第 1 个，防凑份数）
7. 「结算」开奖：未领完需**再点一次确认**（未领者按 0 点输光）→ 播报「本局结果，总分N分」+ **结算结果表格图**（普通消息不带 @全体；v3.1 起无逐人 @ 开奖；R8 起表含红包金额列、改/补值标「（代填）」，撑庄尾行 = `撑 X（红包0.40 点4）：结算 -10`）→ 开奖表补「结果」列（保留到下次开局）
...
- 回放：本局内下注、撑庄与群聊天全程计入对局回放（`record_chat`；v3.1 起红包领取/开奖不逐人回执），随结算上报总后台
- 总后台未知会员/停用/余额不足 → 该事件按 delta=0 记入时间线并随结算回报 `warning` 明细（流水额为 0，不影响其他事件的积分与流水）
...
- **积分与流水分开记**（2026-09-13）：`delta` 决定积分变动，`flow` 只记流水额。撑庄局流水按「下注半额」恒正两侧对等（下注 100 → 玩家侧 50 + 庄家侧 50；平局也记流水、不动积分），大吃小不带 `flow`（总后台按 `delta` 全额兜底）。上报时只在玩法给了 `flow` 时才下发该键——**不给 ≠ 传 0**
- 自测：`python play_rules/rule_fuhe.py`（76 断言：下注/撑庄/封盘/点数比较/大吃小/撑庄结算/作废向量 + 流水半额与恒正对等；R7 起含封盘/结算 img 结构化表格断言、发红包尾巴移除断言）
